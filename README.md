...
自动计算 **增粉**，写入你自己的腾讯文档表格中以当日日期命名的
sheet（在控制台「设置」页粘贴表格链接即可绑定，**表格 ID 只存本机
settings.local.yaml，不进 git**），7 列表头：`账号所有人 | 所属平台 | 账号名称 | 阅读/播放量 | 内容数 | 增粉 | 累计粉丝`
...
- **快手**: 匿名访问是空壳, `python main.py login kuaishou` 任一账号
  登录后即可看任意公开主页
- **长桥**: 帖子在「原创」tab(自动点击切换), 日期格式 `9 月 15 日`;
  浏览量平台不提供 → 该列人工填写
- **老虎**: 匿名全可见(粉丝/帖子数/日期/浏览量——卡片底部"数字·评论"的
  数字即浏览量, 由用户反馈纠错补上)
...
| 公众号 | `weixin_gzh` | **手动填写型** | - | - | - | 无法抓取, 表格每日生成该行, 数据手动填 |
| 视频号 | `weixin_sph` | **手动填写型** | - | - | - | 同上 |
...
- 雪球/同花顺/知乎/微博/长桥：浏览量仅号主可见(或平台无此机制) → 该列**人工填写**(自动值/0不覆盖手填, 重跑不冲掉)
