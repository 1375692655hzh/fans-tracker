...
| 某平台抓取全失败 | 控制台「账号管理」页顶部「登录状态」面板：点「去登录」弹窗扫码一次（等效 `python main.py login 平台名`）；失败截图在 `logs/screenshots/` |
| 不知道某平台登录态还有效吗 | 「登录状态」面板点「检测」，绿=已登录 / 红=已失效 / 黄=无法判断 |
